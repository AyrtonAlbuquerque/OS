...
/*                                          Timed Handles                                         */
/* ---------------------------------------------------------------------------------------------- */
function DestroyEffectTimed takes effect whichEffect, real timeout returns nothing
endfunction

function DestroyLightningTimed takes lightning whichLightning, real timeout returns nothing
endfunction

function RemoveWeatherEffectTimed takes weathereffect whichEffect, real timeout returns nothing
endfunction

function RemoveItemTimed takes item whichItem, real timeout returns nothing
endfunction

function RemoveUnitTimed takes unit whichUnit, real timeout returns nothing
endfunction

function DestroyUbersplatTimed takes ubersplat whichUberSplat, real timeout returns nothing
endfunction
...
function SetUnitBonus takes unit u, integer bonus_type, real amount returns real
    if bonus_type <= BONUS_SIGHT_RANGE then
        return NewBonus.Set(u, bonus_type, R2I(amount), false)
    else
        return NewBonus.setR(u, bonus_type, amount, false)
    endif
endfunction

function RemoveUnitBonus takes unit u, integer bonus_type returns nothing
    if bonus_type <= BONUS_SIGHT_RANGE then
        call NewBonus.Set(u, bonus_type, 0, false)
    else
        if bonus_type == BONUS_CRITICAL_DAMAGE then
            call NewBonus.setR(u, bonus_type, 1, false)
        else
            call NewBonus.setR(u, bonus_type, 0, false)
        endif
...
function AddUnitBonus takes unit u, integer bonus_type, real amount returns real
    if bonus_type <= BONUS_SIGHT_RANGE then
        return NewBonus.add(u, bonus_type, R2I(amount))
    else
...
function RegisterBonusTypeEvent takes integer bonus_type, code c returns nothing
    call NewBonus.register(c, bonus_type)
endfunction
...
function SetBonusType takes integer bonus_type returns nothing
    local integer last
    ...
    if bonus_type >= BONUS_DAMAGE and bonus_type <= last then
        set NewBonus.type[NewBonus.event] = bonus_type
    endif
endfunction

function GetBonusAmount takes nothing returns real
    if NewBonus.type[NewBonus.event] <= BONUS_SIGHT_RANGE then
        return I2R(NewBonus.integer[NewBonus.event])
    else
...
function SetBonusAmount takes real amount returns nothing
    if NewBonus.type[NewBonus.event] <= BONUS_SIGHT_RANGE then
        set NewBonus.integer[NewBonus.event] = R2I(amount)
    else
